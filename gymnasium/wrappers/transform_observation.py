...
class GrayscaleObservation(
    ...
    def __init__(self, env: gym.Env[ObsType, ActType], keep_dim: bool = False,hi = 255):
        ...
        assert (
            ...
            and np.all(env.observation_space.high == hi)
        ...
        if keep_dim:
            ...
            new_observation_space = spaces.Box(
                low=0, high=hi, shape=env.observation_space.shape[:2], dtype=np.uint8
            )
            TransformObservation.__init__(
                self,
                env=env,
...
class ResizeObservation(
    ...
    def __init__(self, env: gym.Env[ObsType, ActType], shape: tuple[int, int],hi = 255):
